...
for pixel_start_r= k+1:k+r
    for pixel_start_c= k+1:k+c
        G_x = Fx_o(pixel_start_r-k:min(pixel_start_r+k, size(Fx_o, 1)), pixel_start_c-k:min(pixel_start_c+k, size(Fx_o, 2)));
        G_y = Fy_o(pixel_start_r-k:min(pixel_start_r+k, size(Fy_o, 1)), pixel_start_c-k:min(pixel_start_c+k, size(Fy_o, 2)));
        G=[G_x(:) G_y(:)];
        [~,~, V]=svd(G);%PCA
