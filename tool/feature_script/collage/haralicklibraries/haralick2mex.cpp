    ...
    for (j = block_start_col; j < block_end_col; j++)
        for (i = block_start_row; i < block_end_row; i++) {
            ...
            if (center_value == background)
                continue;
//next loop allow for all angels, then sum them
            ...
            for (l = d_start_col; l <= d_end_col; l++)
                for (k = d_start_row; k <= d_end_row; k++) {
                    ...
                    if (image[pixind]!=background && pixind!=centerind)
                        comtx[center_value + (int) (image[pixind]+0.5)*graylevels] += 1;//0.5?
                }
        }
}
    ...
    for (j=0; j<cols; j++) {
        for (i=0; i<rows; i++) {
            ...
            for (k=0, ii=0; k<nbins; k++) {
                if (comtx[k]>0) {
                    p[k]=comtx[k]/cosum;//probability
                    pnz[ii]=p[k];//p_non zero
                    nzcomtx[ii]=comtx[k];
                    hi[ii]=k % graylevels;//column
                    hj[ii]=(int) floor((float) k/(float) graylevels);//row
                    himhj[ii]=hi[ii]-hj[ii];//column - row
                    hiphj[ii]=hi[ii]+hj[ii];//column + row
                    ii++;
                } else {
                    p[k]=0;
                }
            ...
            for (k=0, ii=0; ii<graylevels; ii++)
                for (jj=0; jj<graylevels; jj++, k++) {
                    py[ii]+=p[k];//sum(row)//row|row|row|...
                    px[jj]+=p[k];//sum(column)//column|column|column|...
                }
            /*
            ...
            for (ii=0; ii<graylevels; ii++) {
                sigma_x+=pow(ii-mu_x,2) * px[ii];//variance
