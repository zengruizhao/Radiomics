...
for k=1:shape(3)    
    ...
    if(exist('bwdist', 'builtin') && aspect(1)==aspect(2))
        ...
        xlower=Inf(shape(1:2)); 
        
        xlower(1,bwXY(1,:))=1;    % fill in first row
        for i=2:shape(1)
            xlower(i,:)=xlower(i-1,:);  % copy previous row
            xlower(i,bwXY(i,:))=i;% unless there is pixel
        end
        
        % reference nearest bwXY "on"-pixel in x direction upward:
        xupper=Inf(shape(1:2));
        
        xupper(end,bwXY(end,:))=shape(1);
        for i=shape(1)-1:-1:1
            xupper(i,:)=xupper(i+1,:);
            xupper(i,bwXY(i,:))=i;
        end
        ...
        idx=find(~bwXY); [x,~]=ind2sub(shape(1:2),idx);
...
D1=cell(size(D));
for k=1:shape(3), D1{k}=Inf(shape(1:2)); end
% these will be the references to parabolas defining the envelop
DK=cell(size(D));
for k=1:shape(3), DK{k}=Inf(shape(1:2)); end
...
for i=2:length(ind)
    ...
    s(dtmp(ind(i))).PixelIdxList=idx(ind(i-1)+1:ind(i)); %#ok<AGROW>
end