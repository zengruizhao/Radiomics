...
function extractfullfeature_OroCavity_v3_identify_case(paraC)
...
%     dirIM(1:3)=[];
        ...
        strESMask='C:\Nutstore\Nutstore\training40_downby8_png\';
%         dirIM=dir(strIMPath);
%         dirIM(1:2)=[];
    else
        strIMPath='../Training40/';
        strPath='../Training40_features/';
        strESMask='../Training40_downby8_png/';
%         dirIM=dir(strIMPath);
%         dirIM(1:2)=[];
    end
end
...
if exist(str,'file')~=2
    ...
    [feats_epi,description_epi,~] = Lextract_CCM_features(bounds_epi,I_norm,properties,para);
    ...
    [feats_stroma,description_stroma,~] = Lextract_CCM_features(bounds_stroma,I_norm,properties,para);
...
if exist(str,'file')~=2
    ...
    [feats,description,~] = Lextract_CCM_features(bounds,I_norm,properties,para);
