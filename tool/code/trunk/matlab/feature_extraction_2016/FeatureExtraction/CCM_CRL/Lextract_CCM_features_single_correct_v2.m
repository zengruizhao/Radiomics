...
% multi-loop senario!!!
% this one correct the way of coocurrence matrix calculation
function [allCCM,CCM_description,CCGinfo] = Lextract_CCM_features_single_correct_v2(bounds,I,properties,a,r)
%% buid ccg
...
centroidc = [bounds.centroid_c];
for j = 1:length(centroidc)-1
    for k = j+1:length(centroidc)
        edges(k,j) = edges(j,k);
    end
end
...
% feature_names={'Area','Eccentricity','EquivDiameter','Solidity','Perimeter','Circularity',...
%     'MeanIntensity','IntensityDeviation','MeanInsideBoundaryIntensity','InsideBoundaryIntensityDeviation',...
%     'MeanOutsideBoundaryIntensity','OutsideBoundaryIntensityDeviation'};
% feature_max=[2000,   1,  60,    1, 180,    1, 180,  60,  180, 60,  180, 60];
% feature_min=[ 200, 0.4,  20, 0.86,  50, 0.85,  40,  10,   50, 10,   50, 10];
