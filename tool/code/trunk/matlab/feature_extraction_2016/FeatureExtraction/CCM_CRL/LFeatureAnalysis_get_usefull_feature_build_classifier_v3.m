% log:
% v3 is used after the first try of classifier using LDA and SVM
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%lock down top features for whole data without CCM
set_name={'CCM'};
[~,data_noccm,feature_list_noccm]=Lfind_featureidx_by_excluded_name(set_name,data_all,feature_list_all);
...
%% feature ranking using data without CCM
...
%% remove feature have high correlation
set_Feature=Lpick_top_n_features_with_pvalue_correlation(data_noccm,set_Feature,para.num_top_feature,para.correlation_factor);
set_Feature_name=feature_list_t(set_candiF)';
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%lock down top features for whole data within CCM with a=0.4-0.42
set_name={'CCM'};
[~,data_ccm,feature_list_ccm]=Lfind_featureidx_by_name(set_name,data_all,feature_list_all);

%% focus on specific CCG resolution
set_usful_a_names={'0.38','0.42','0.40'};
[~,data_ccm_useful_a,feature_list_ccm_useful_a]=Lfind_featureidx_by_name(set_usful_a_names,data_ccm,feature_list_ccm);

dataw_epistroma=simplewhiten(data_ccm_useful_a);
feature_list_epistroma=feature_list_ccm_useful_a;
para.correlation=0.9;
% [TTidx,p] = prunefeatures_new(dataw_epistroma, labels, 'ttestp');
% idx_TTest=TTidx(p<0.05);
% % set_p=p(1:K);
...
set_candiF_wilcoxon=Lpick_top_n_features_with_pvalue_correlation(dataw_epistroma,idx_wilcoxon,18,para.correlation);
set_fff_wilcoxon=feature_list_epistroma(set_candiF_wilcoxon)';
%% feature ranking using data within CCM with a=0.4-0.42
