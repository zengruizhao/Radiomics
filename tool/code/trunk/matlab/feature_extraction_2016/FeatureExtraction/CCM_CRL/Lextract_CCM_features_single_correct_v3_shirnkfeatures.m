...
% v3 reduce the number of features significantly
function [allCCM,CCM_description,CCGinfo] = Lextract_CCM_features_single_correct_v3(bounds,I,properties,a,r)
%% buid ccg
...
feature_names={'Eccentricity','EquivDiameter',...
    'MeanIntensity',...
    'OutsideBoundaryIntensityRange'};

feature_max=[ 1,   60,   180, 180];
feature_min=[ 0.4, 20,   40,  40];
...
for i=1:length(feature_names)
    ...
    if ~isempty(feats) && length(feats)>3
        ...
        curCCM=[];
        curCCM=[median(all_feat) std(all_feat) range(all_feat)];
        %         idx_ccm=1;
        ...
        allCCM=[allCCM zeros(1,39)];
    end
end

%% feature names organization
count = 1;
modifier = [{'median'} {'std'} {'range'}];
