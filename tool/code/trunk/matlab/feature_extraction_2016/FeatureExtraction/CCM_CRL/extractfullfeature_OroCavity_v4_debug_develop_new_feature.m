...
function extractfullfeature_OroCavity_v4_debug_develop_new_feature(paraC)
...
str=sprintf('%sCRLMFeatures_IM%s_epistroma_test.mat',strPath,dirIM(i).name);
...
if paraC.epi
    ...
    bounds_epi.centroid_r=ctemp(2:2:end);
    bounds_epi.nuclei=nuclei(nuclei_label);
    [allCCM,CCM_description]=Lextract_CRL_features_single(bounds_epi,I_norm,properties,paraC);
else
    ctemp=[properties(~nuclei_label).Centroid];
    bounds_stroma.centroid_c=ctemp(1:2:end);
    bounds_stroma.centroid_r=ctemp(2:2:end);
    bounds_stroma.nuclei=nuclei(~nuclei_label);
    [allCCM,CCM_description]=Lextract_CRL_features_single(bounds_stroma,I_norm,properties,paraC);
end
