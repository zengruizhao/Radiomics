...
% V6 extract CRL features, note that CRL does not classify cell into types
% V6 also extract Hosoya features
function nucleiseg_nucleiclusterfinding_extractfullfeature_OroCavity_v6(idxBegin,idxEnd)
...
for i=idxBegin:idxEnd   
    ...
    str=sprintf('%sCRLFeatures_IM%s_epistroma.mat',strPath,dirIM(i).name);
    ...
    if exist(str,'file')~=2
        ...
        [feats_epi,description_epi,~] = Lextract_CRL_features_v2(bounds_epi,I_norm,properties,para);
        ...
        [feats_stroma,description_stroma,~] = Lextract_CRL_features_v2(bounds_stroma,I_norm,properties,para);
        
        save(str,'feats_stroma','description_stroma','feats_epi','description_epi','-v7.3');
        disp('CRL features result in epi/stroma saved');
    else
        disp('CRL features file in epi/stroma found, loading it');
        load(str);
    end
    %% extract Hosoya features
     str=sprintf('%sHosoyaFeatures_IM%s_epistroma.mat',strPath,dirIM(i).name);
     para.CGalpha_min=0.38; para.CGalpha_max=0.5;
     para.alpha_res=0.02;% larger the alpha, sparse the graph
     para.radius=0.2;
     ...
     if exist(str,'file')~=2
        ...
        for f=1:length(set_alpha)
            ...
            [hosoya_all_epi{f}, CCGinfo_epi{f}]= Lextract_Hosoya_features(bounds_epi,curpara.alpha,curpara.radius);
            [hosoya_all_stroma{f}, CCGinfo_stroma{f}]= Lextract_Hosoya_features(bounds_stroma,curpara.alpha,curpara.radius);
        end
    ...
    %% extract CRL features
    str=sprintf('%sCRLFeatures_IM%s.mat',strPath,dirIM(i).name);
    para.CGalpha_min=0.38; para.CGalpha_max=0.5;
    para.alpha_res=0.02;% larger the alpha, sparse the graph
    para.radius=0.2;
    ...
    if exist(str,'file')~=2
        ...
        disp('CRL features file found, loading it');
        load(str);
    end   
     %% extract Hosoya features
