function [collagefeats, collage_description,display]=compute_CoLlAGe2D_update(origImage, tumorMask, MPP, showfigures,ws)
...
factor = 0.5/MPP;
ws = 2*ws+1;
gws = ws;% outer img than inner img
hws = ws;% filter window size
hdist = 1;
N = 64;
...
for i = 1:numel(size(volfeats,3))
    ...
    display.texture_img{i} = entrop_map.*BWorig;
    h = histogram(feature_concat);
