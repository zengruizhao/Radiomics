function [Iregd,rigidparams,varargout] = rigidreg2(Itemplate,Itarget,N,varargin)
...
ip.addParamValue('paramweights', [125; 4000; 4000; .25; .25; 125], @(x)isfloat(x) && numel(x)>=nminparams && numel(x)<=nmaxparams);
...
% elseif scales==1 && nparamoffsets==5, % assume last element is theta2
%     paramoffsets(end:end+1)=paramoffsets(end-1:end); % and duplicate the scale
end
% if scales==1 and nparamoffsets==5, we assume two scale offsets and no theta2
...
% objdef2d(rigidparams,I,Itemp,bgvalue,N,nodisp,paramweights,measure,interpmethod)
fprintf('Measure: %s.  Interpolation: %s.\n',ip.Results.measure,ip.Results.interpmethod);
rigidparams=fminsearch(@objdef2drigid, startparams_objdef, optoptions,...
	double(Itarget), double(Itemplate), ip.Results.bgvalue, N,...
    ip.Results.nodisp, paramweights, ip.Results.measure,...
    ip.Results.interpmethod, paramoffsets, preM, ip.Results.coordshift, scales==1);
...
% IregdNN=imdefnn(rigidparams,Itarget,[],cast(0,class(Itarget)));
[Iregd]=imdef(rigidparams, Itarget, interpmethod, bgvalue, preM, ip.Results.coordshift);
% [IregdNN,M]=imdef(rigidparams, Itarget, 'nearest', bgvalue);
[IregdLIN,M]=imdef(rigidparams, Itarget, 'linear', bgvalue, preM, ip.Results.coordshift);
...
% Generate overlay and checker images for all corresponding slices, first attribute
if nargout>5 || ~ip.Results.nodisp,
    overlayweight=0.5;
    [Itemplate_scaled,drange,dmin]=rescale(Itemplate(:,:,:,1));
    Iregd_crop_scaled=(Iregd_crop(:,:,:,1)-dmin)/drange;
    overlayims=rescale(Iregd_crop_scaled+overlayweight*Itemplate_scaled);
    if nargout>5, varargout{4}=overlayims; end
    if nargout>6 || ~ip.Results.nodisp,
        checkerims=imchecker(Iregd_crop_scaled,Itemplate_scaled,round(size(Iregd_crop,1)/25));
        if nargout>6, varargout{5}=checkerims; end
    end
end
...
if ~ip.Results.nodisp,
    ...
    subplot(2,3,4); imdisp(overlayims(:,:,1)); title('Image Overlaying');
    subplot(2,3,5); imdisp(checkerims(:,:,1)); title('Checker Image'); 
end
