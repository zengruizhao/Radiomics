...
%Inew(Inew>(N-1))=N-1; % threshold if larger than N
    ...
    switch lower(measure)
        ...
        case 'mmi'
            ...
            if (sumall(J(:,:,K+1))>0) % last slice has a different target
                err = err +  1.0*addedElem*mimex(J(:,:,number_many_slices+1),Inew,N); % use addedElement to weight the (K+1)th slice to have the same weight
            end
