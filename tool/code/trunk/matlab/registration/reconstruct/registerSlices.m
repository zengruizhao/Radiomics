...
for (i = 1:size(vol,3))
   ...
   t = createReconstructionTarget(d_reg, i, target, Kslices, score_forward,rigid_params);
    ...
    if (size(orient_t,1)>0) % a target exist    
        ...
        %%%
        newparam(1) = ((degtorad(ang_diff))+rigid_params(i,1))/2;
        newparam(2) = ((com_t(1)-com_m(1))+rigid_params(i,2))/8;
        newparam(3) = ((com_t(2)-com_m(2))+rigid_params(i,3))/8;
        %newparam(4) = 1;
        %newparam(5) = 1;
        ...
        %if (maxall(moved_mask+t_proj_mask)==1) % they don't overlap; so just use the old params 
            ...
            newparam(2) = rigid_params(i,2)/4;
            newparam(3) = rigid_params(i,3)/4;
