function status = writeReconstruction(path_out, prefix_masks, reconstruction, reconstructionR, reconstructionG,...
    ...
    newVol(:,:,newRange) = reconstructionR;
    if (flipOnZ==1) newVol = flipLRvol(newVol); end
    mha_write_volume([path_out, 'reco_R.mha'],  newVol, voxel_size);

    newVol(:,:,newRange) = reconstructionG;
    if (flipOnZ==1) newVol = flipLRvol(newVol); end
    mha_write_volume([path_out, 'reco_G.mha'],  newVol, voxel_size);
    
    newVol(:,:,newRange) = reconstructionB;
    if (flipOnZ==1) newVol = flipLRvol(newVol); end
    mha_write_volume([path_out, 'reco_B.mha'],  newVol, voxel_size);
