function [skeleton,skeleton_rad]=get_skeleton(image_mask,image_edge,dim,atom_num,percentile,norm_num)
...
[backbone,map]=getskel_grad(image_mask,percentile,dim);
size_map=size(map);

%initialize skeleton radius matrix
skeleton_rad=zeros(size_map);

%Find all backbone points
[i]=find(backbone);
...
if strcmp(atom_num,'full');
    ...
    atom_step=(length(i)/atom_num);
end

%iterate through each atom point selected
    j_temp=1;    
for k_temp=atom_step:atom_step:length(i);
        k=min([ceil(k_temp), length(i)]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                skeleton.atom{j_temp}.location=[x,y,z];
                skeleton.atom{j_temp}.radius=rad;
                skeleton.atom{j_temp}.norms=norms(:,1:min(norm_num,size(norms,2)));
                j_temp=j_temp+1;
            end
end
