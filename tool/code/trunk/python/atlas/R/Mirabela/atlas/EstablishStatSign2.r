#source("C:\\Users\\mirab\\Documents\\papers\\ccipd_code_newPath\\trunk\\R\\Mirabela\\atlas\\EstablishStatSign2.R")

tab_newRat = read.table("C:\\Mirabela\\Projects\\AtlasComparison\\Results\\VolumeComparisonAfterRegistrationND.txt", header=TRUE, sep = ",")
...
for (i in 1:dim(tab_newRat)[1])
{
	...
	{
		caseID = paste(substr(tab_newRat[i,1],1,9),sep='')
	}
	else{
	
		if (substr(tab_newRat[i,1],1,5)=="CAP-p")
		{
			caseID = paste(substr(tab_newRat[i,1],11,14),sep='')
		}
		else
		{
			caseID = substr(tab_newRat[i,1],4,7)
		}
	}
	...
	for (j in 1:length(disSbj))
	{
		if (caseID==disSbj[j])
			...
			found  = 1
			#print (paste("  ",i, caseID,j,disSbj[j],caseID==disSbj[j]))
