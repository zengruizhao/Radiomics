...
if not opt then
   ...
   cmd:option('-dsperc',3,'percentage of dataset (stratified)')
...
--prefixdatapath="/home/aacruz/Experiments/datasets/mitosis/hpfs/"
...
if opt.dataset == "icpr2012+H" then
   ...
   pixDist = 21.99  
   -- ICPR: 20.4, MICCAI: 30
   train_img_dir = prefixdatapath .. "icpr2012/H/train"
...
elseif opt.dataset == "icpr2012+A" then
   ...
   pixDist = 20.4 
   -- ICPR: 20.4, MICCAI: 30
   train_img_dir = prefixdatapath .. "icpr2012/A/train"
...
elseif opt.dataset == "miccai2013" then
   ...
   pixDist = 30    
   -- ICPR: 20.4, MICCAI: 30
   train_img_dir = prefixdatapath .. "miccai2013/train"
...
for file in paths.files(train_img_dir) do
   ...
   end  -- end if if is png
end -- for list of images
...
--If random selection is needed uncomment next line and coment the following one
...
if opt.visualize then
   ...
   k = 1
   samples_mitosis={}
   print '==> visualizing mitotic examples'

   while k<trainData:size() and j<100 do
      if trainData.labels[{k}] == 1 and train_dupid[k]==1 then
	 samples_mitosis[j] = trainData.data[{k}]
	 j=j+1
	 print(j)
      end
      k=k+1
   ...
   image.display{image=samples_mitosis, nrow=10, legend='Mitosis examples'}
end
...
--local filename = paths.concat(opt.save, 'mean_' .. opt.dataset .. '.dat')
--os.execute('mkdir -p ' .. sys.dirname(filename))
--print('==> saving mean to '.. filename)
--torch.save(filename, mean)

--filename = paths.concat(opt.save, 'std_' .. opt.dataset .. '.dat')
--os.execute('mkdir -p ' .. sys.dirname(filename))
--print('==> saving std to '.. filename)
